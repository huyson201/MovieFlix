        ...
        <div className={`hero-slide`} style={{ backgroundImage: `url(${originalImage(movie.backdrop_path)})` }}>
            <Wrapper className='h-full relative z-10'>
                <div className="slide-content w-full md:w-[65%] pr-6 ">
                    <Link to={`${urlMap[movie.media_type]}/${encodeURIComponent((movie.name || movie.title).toLowerCase()).replace(/%20/g, '-') || "na"}/${movie.id}`} className="movie-name duration-300 transition-colors hover:text-dark-teal text-3xl md:text-4xl  text-white font-bold  drop-shadow-lg block pr-6">{movie.name || movie.title}</Link>
