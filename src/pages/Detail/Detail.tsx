...
const Detail = ({ mediaType }: Props) => {
    ...
    return (
        <div className='detail-page' >
            <div className="detail" style={{ backgroundImage: `url(${originalImage(data.data.backdrop_path)})` }}>
                <Wrapper className='relative z-[1] flex flex-col md:flex-row gap-8 md:gap-16 py-5'>
                    <div className="detail-card overflow-hidden self-center rounded-2xl w-60">
                        <img src={originalImage(data.data.poster_path)} alt={(data.data as DetailMovie).title || (data.data as DetailTV).name || ''} />
                    </div>
                    <div className="detail-content text-white md:flex-1">
                        ...
                        <div className='flex items-center gap-6 flex-wrap mt-6'>
                            {
                                data.data.genres.map((genre, index) => {
                                    return (
                                        <span key={genre.id.toString() + index.toString()} className='genre-items text-sm border border-white rounded-3xl py-1 px-2'>{genre.name}</span>
                        ...
                        <div className='flex items-center mt-5 gap-x-8 gap-y-4 flex-wrap'>
                            {queryCast.data && queryCast.data?.data.cast.slice(0, 4).map(cast => {
